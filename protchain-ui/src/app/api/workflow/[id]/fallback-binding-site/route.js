...
/**
 * POST handler for running a fallback binding site detection algorithm
 * This is used when fpocket and LIGSITE fail to detect binding sites
 * 
 * The algorithm implemented here is a simplified geometric approach based on
 * concepts from the following academic papers:
 * 
  ...
  try {
    ...
    if (ligandAtoms.length > 0) {
      ...
      Object.keys(ligandResidues).forEach((ligandKey, index) => {
        ...
        const volume = estimateVolume(ligandAtomList);
        const score = 0.8; // High confidence since we're using a known ligand
      ...
      Object.keys(residues).forEach(key => {
        residueCentroids[key] = calculateCentroid(residues[key]);
      });
      ...
      clusters.forEach((cluster, index) => {
        ...
        cluster.residues.forEach(res => {
          allAtoms.push(...residues[res.key]);
        });
        
        const centroid = calculateCentroid(allAtoms);
        const volume = estimateVolume(allAtoms);
        const score = Math.min(0.5 + (cluster.residues.length / 40), 0.9);
        
        // Get residue information
        const siteResidues = cluster.residues.map(res => {
          const [resName, chainId, resNum] = res.key.split('_');
          return {
            name: resName,
            number: parseInt(resNum),
            chain: chainId
          };
        });
        
        const druggability = calculateDruggability(allAtoms);
        const hydrophobicity = calculateHydrophobicity(allAtoms);
    ...
    resultsData.binding_site_analysis.method = 'fallback';
    ...
    return NextResponse.json({
      status: 'success',
      message: 'Fallback binding site detection completed successfully',
